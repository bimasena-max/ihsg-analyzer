...
import {
  calcRSISeries, calcEMA, calcMACD, calcBollinger, calcATR,
} from './indicators.js';
...
export function buildLastBarFeatures({ closes, highs, lows, vols, dates }) {
  ...
  // RSI14 butuh pemanasan +14 bar, EMA50 secara praktik perlu jauh lebih
  // panjang untuk stabil, dd_from_high pakai window 252 (rolling 1 tahun).
  // 260 bar (~1 tahun bursa) adalah syarat minimum yang sama dipakai
  // predict.py: `if df is None or len(df) < 260: continue`.
  if (n < 260) {
    return { features: null, vector: null, missing: ['data_kurang_dari_260_bar'] };
  }
  ...
  const bbLast = bb[i];
  // features.py: bb_pos = (c - (ma20 - 2*sd20)) / (4*sd20)
  //   ma20 - 2*sd20 == bbLast.lower ; 4*sd20 == bbLast.upper - bbLast.lower
  let bb_pos = null, bb_width = null;
  if (bbLast) {
    const span = bbLast.upper - bbLast.lower;
    bb_pos = span !== 0 ? (c[i] - bbLast.lower) / span : null;
    bb_width = bbLast.width;
  }
